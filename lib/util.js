...
const util = {
  ...
  fetch: (options = { cwd: config.rootDir }) => {
    options = mergeWithDefault(options)
    util.run('git', ['-C', options.git_dir, 'fetch', '--all', '--tags'], options)
  },

  setVersion: (version, options = { cwd: config.rootDi}) => {
    if (!options.git_dir) options.git_dir = '.'
    util.run('git', ['-C', options.git_dir, 'clean', '-f'], options)
    util.run('git', ['-C', options.git_dir, 'reset', '--hard', version], options)
  },

  setDepVersion: (dir, version) => {
    const options = { git_dir: dir }
    util.fetch(options)
