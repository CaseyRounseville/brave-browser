...
const util = {
  run: (cmd, args = [], options = {}) => {
    ...
    if (config.prepend_path) {
      ...
      options.env.PATH = config.prependPath(options.env.PATH||process.env.PATH, config.prepend_path)
      options.env.Path = config.prependPath(options.env.Path||process.env.Path, config.prepend_path)
    }
  ...
  submoduleSync: (options = { cwd: config.rootDir }) => {
    options = mergeWithDefault(options)
    util.run('git', ['-C', options.git_repo, 'submodule', 'sync'], options)
    util.run('git', ['-C', options.git_repo, 'submodule', 'update', '--init', '--recursive'], options)
  ...
  fetch: (options = {}) => {
    options = mergeWithDefault(options)
    util.run('git', ['-C', options.git_repo, 'fetch', '--all', '--tags'], options)
  },

  setVersion: (version, options = {}) => {
    util.run('git', ['-C', options.git_repo, 'clean', '-f'], options)
    util.run('git', ['-C', options.git_repo, 'reset', '--hard', version], options)
  },

  setDepVersion: (dir, version) => {
    const options = { git_repo: dir }
    util.fetch(options)
